...
! function(t, e) {
    ...
    function n(e) {
        ...
        if (n.SYSTEM.loaded || n._loadSystem(), this.config = x.clone(n.DEFAULTS), x.deepmerge(this.config, e), !e.container) throw new E("No value given for container.");
        if (!n.SYSTEM.isCanvasSupported) throw new E("Canvas is not supported.");
        if (!(n.SYSTEM.isWebGLSupported && this.config.webgl || x.checkTHREE("CanvasRenderer", "Projector"))) throw new E("Missing Three.js components: CanvasRenderer, Projector. Get them from three.js-examples package.");
        ...
        this.config.cache_texture && (!x.isInteger(this.config.cache_texture) || this.config.cache_texture < 0) && (this.config.cache_texture = n.DEFAULTS.cache_texture, console.warn("PhotoSphereViewer: invalid value for cache_texture")), "panorama_roll" in this.config && (this.config.sphere_correction.roll = this.config.panorama_roll, console.warn("PhotoSphereViewer: panorama_roll is deprecated, use sphere_correction.roll instead")), "gyroscope" in this.config && console.warn("PhotoSphereViewer: gyroscope is deprecated, the control is automatically created if DeviceOrientationControls.js is loaded"), this.config.min_fov = x.bound(this.config.min_fov, 1, 179), this.config.max_fov = x.bound(this.config.max_fov, 1, 179), null === this.config.default_fov ? this.config.default_fov = this.config.max_fov / 2 + this.config.min_fov / 2 : this.config.default_fov = x.bound(this.config.default_fov, this.config.min_fov, this.config.max_fov), this.config.default_long = x.parseAngle(this.config.default_long), this.config.default_lat = x.parseAngle(this.config.default_lat, !0), this.config.sphere_correction.pan = x.parseAngle(this.config.sphere_correction.pan, !0), this.config.sphere_correction.tilt = x.parseAngle(this.config.sphere_correction.tilt, !0), this.config.sphere_correction.roll = x.parseAngle(this.config.sphere_correction.roll, !0), null === this.config.anim_lat ? this.config.anim_lat = this.config.default_lat : this.config.anim_lat = x.parseAngle(this.config.anim_lat, !0), this.config.longitude_range && (this.config.longitude_range = this.config.longitude_range.map(function(t) {
            ...
            this.config.templates[t] || (this.config.templates[t] = n.TEMPLATES[t]), "string" == typeof this.config.templates[t] && (this.config.templates[t] = o.template(this.config.templates[t]))
        ...
        this.zoom(s - 2 * (s - 50)), this.prop.move_speed = t.Math.degToRad(this.config.move_speed / n.SYSTEM.pixelRatio), this.rotate({
            longitude: this.config.default_long,
            latitude: this.config.default_lat
        }), this.loader = new a(this), this.loader.hide(), this.navbar = new h(this), this.navbar.hide(), this.hud = new r(this), this.hud.hide(), this.panel = new l(this), this.tooltip = new d(this.hud), this.notification = new c(this), this._bindEvents(), this.config.panorama && this.setPanorama(this.config.panorama), this.once("render", function() {
        ...
        }.bind(this)), n.SYSTEM.touchEnabled.then(function() {
            this.container.classList.add("psv-is-touch")
        }.bind(this))
    }
    i.mixin(n), n.prototype._run = function(t) {
        ...
        if (!this.config.usexmpdata) return e.resolved(null);
        var i = e(),
        ...
        return o.onreadystatechange = function() {
            if (4 === o.readyState) {
                if (200 !== o.status && 201 !== o.status && 202 !== o.status && 0 !== o.status) throw this.container.textContent = "Cannot load image", new E("Cannot load image");
                this.loader.setProgress(100);
                var t = o.responseText,
                    e = t.indexOf("<x:xmpmeta"),
                    s = t.indexOf("</x:xmpmeta>"),
                    r = t.substring(e, s);
                if (-1 === e || -1 === s || -1 === r.indexOf("GPano:")) i.resolve(null);
                else {
                    var a = {
                    ...
                    a.full_width && a.full_height && a.cropped_width && a.cropped_height ? i.resolve(a) : (console.warn("PhotoSphereViewer: invalid XMP data"), i.resolve(null))
                }
            } else 3 === o.readyState && this.loader.setProgress(n += 10)
        }.bind(this), o.onprogress = function(t) {
            if (t.lengthComputable) {
                var e = parseInt(t.loaded / t.total * 100);
                e > n && (n = e, this.loader.setProgress(n))
            }
        }.bind(this), o.onerror = function() {
            throw this.container.textContent = "Cannot load image", new E("Cannot load image")
        }.bind(this), o.open("GET", t, !0), o.send(null), i.promise
    }, n.prototype._loadTexture = function(t) {
        var e = [];
        if (Array.isArray(t)) {
            if (6 !== t.length) throw new E("Must provide exactly 6 image paths when using cubemap.");
            for (var i = 0; i < 6; i++) e[i] = t[n.CUBE_MAP[i]];
            t = e
        ...
        if (Array.isArray(t)) {
            if (!1 === this.prop.isCubemap) throw new E("The viewer was initialized with an equirectangular panorama, cannot switch to cubemap.");
            return this.config.fisheye && console.warn("PhotoSphereViewer: fisheye effect with cubemap texture can generate distorsions."), this.config.cache_texture === n.DEFAULTS.cache_texture && (this.config.cache_texture *= 6), this.prop.isCubemap = !0, this._loadCubemapTexture(t)
        }
        if (!0 === this.prop.isCubemap) throw new E("The viewer was initialized with an cubemap, cannot switch to equirectangular panorama.");
        return this.prop.isCubemap = !1, this._loadEquirectangularTexture(t)
    }, n.prototype._loadEquirectangularTexture = function(i) {
        if (this.config.cache_texture) {
            var o = this.getPanoramaCache(i);
            if (o) return this.prop.pano_data = o.pano_data, e.resolved(o.image)
        }
        return this._loadXMP(i).then(function(o) {
            ...
            return r.load(i, function(e) {
                a = 100, this.loader.setProgress(a), this.trigger("panorama-load-progress", i, a), !o && this.config.pano_data && (o = x.clone(this.config.pano_data)), o || (o = {
                    full_width: e.width,
                ...
                if (1 !== h || o.cropped_width !== o.full_width || o.cropped_height !== o.full_height) {
                    var p = x.clone(o);
                    p.full_width *= h, p.full_height *= h, p.cropped_width *= h, p.cropped_height *= h, p.cropped_x *= h, p.cropped_y *= h, e.width = p.cropped_width, e.height = p.cropped_height;
                    var c = document.createElement("canvas");
                    c.width = p.full_width, c.height = p.full_height, c.getContext("2d").drawImage(e, p.cropped_x, p.cropped_y, p.cropped_width, p.cropped_height), r = new t.Texture(c)
                } else r = new t.Texture(e);
                r.needsUpdate = !0, r.minFilter = t.LinearFilter, r.generateMipmaps = !1, this.config.cache_texture && this._putPanoramaCache({
                    panorama: i,
                    image: r,
                    pano_data: o
                }), s.resolve(r)
                ...
                if (t.lengthComputable) {
                    var e = parseInt(t.loaded / t.total * 100);
                    e > a && (a = e, this.loader.setProgress(a), this.trigger("panorama-load-progress", i, a))
                }
            }.bind(this), function(t) {
                throw this.container.textContent = "Cannot load image", s.reject(t), new E("Cannot load image")
            }.bind(this)), s.promise
        }.bind(this))
        ...
        for (var p = function(e, s) {
                h++, r[e] = 100, this.loader.setProgress(x.sum(r) / 6), this.trigger("panorama-load-progress", i[e], r[e]);
                var p = Math.min(s.width, n.SYSTEM.maxTextureWidth / 2) / s.width;
                if (1 !== p) {
                    var c = document.createElement("canvas");
                    c.width = s.width * p, c.height = s.height * p;
                    c.getContext("2d").drawImage(s, 0, 0, c.width, c.height), a[e] = new t.Texture(c)
                } else a[e] = new t.Texture(s);
                this.config.cache_texture && this._putPanoramaCache({
                    panorama: i[e],
                    image: a[e]
                }), 6 === h && (a.forEach(function(e) {
                    e.needsUpdate = !0, e.minFilter = t.LinearFilter, e.generateMipmaps = !1
                }), o.resolve(a))
            }, c = function(t, e) {
                if (e.lengthComputable) {
                    var o = parseInt(e.loaded / e.total * 100);
                    o > r[t] && (r[t] = o, this.loader.setProgress(x.sum(r) / 6), this.trigger("panorama-load-progress", i[t], r[t]))
                }
            }, l = function(t, e) {
                throw this.container.textContent = "Cannot load image", o.reject(e), new E("Cannot load image " + t)
            }, u = 0; u < 6; u++) {
            if (this.config.cache_texture) {
                var d = this.getPanoramaCache(i[u]);
                if (d) {
                    h++, r[u] = 100, a[u] = d.image;
                    continue
                }
            }
            s.load(i[u], p.bind(this, u), c.bind(this, u), l.bind(this, u))
        }
        return 6 === h && o.resolve(a), o.promise
        ...
        this.raycaster = new t.Raycaster, this.renderer = n.SYSTEM.isWebGLSupported && this.config.webgl ? new t.WebGLRenderer : new t.CanvasRenderer, this.renderer.setSize(this.prop.size.width, this.prop.size.height), this.renderer.setPixelRatio(n.SYSTEM.pixelRatio);
        var e = n.SPHERE_RADIUS;
        this.prop.isCubemap && (e *= Math.sqrt(3)), this.config.fisheye && (e += n.SPHERE_RADIUS), this.camera = new t.PerspectiveCamera(this.config.default_fov, this.prop.size.width / this.prop.size.height, 1, e), this.camera.position.set(0, 0, 0), this.scene = new t.Scene, this.scene.add(this.camera), this.prop.isCubemap ? this.mesh = this._createCubemap() : this.mesh = this._createSphere(), this.scene.add(this.mesh), this.canvas_container = document.createElement("div"), this.canvas_container.className = "psv-canvas-container", this.renderer.domElement.className = "psv-canvas", this.container.appendChild(this.canvas_container), this.canvas_container.appendChild(this.renderer.domElement)
        ...
        var i = new t.SphereGeometry(n.SPHERE_RADIUS * e, n.SPHERE_VERTICES, n.SPHERE_VERTICES, -x.HalfPI),
            ...
            s = new t.Mesh(i, o);
        return s.scale.x = -1, s.rotation.x = this.config.sphere_correction.tilt, s.rotation.y = this.config.sphere_correction.pan, s.rotation.z = this.config.sphere_correction.roll, s
    }, n.prototype._createCubemap = function(e) {
        e = e || 1;
        for (var i = new t.BoxGeometry(2 * n.SPHERE_RADIUS * e, 2 * n.SPHERE_RADIUS * e, 2 * n.SPHERE_RADIUS * e, n.CUBE_VERTICES, n.CUBE_VERTICES, n.CUBE_VERTICES), o = [], s = 0; s < 6; s++) o.push(new t.MeshBasicMaterial({
            side: t.BackSide,
            overdraw: n.SYSTEM.isWebGLSupported && this.config.webgl ? 0 : 1
        }));
        var r = new t.Mesh(i, o);
        return r.position.x -= n.SPHERE_RADIUS * e, r.position.y -= n.SPHERE_RADIUS * e, r.position.z -= n.SPHERE_RADIUS * e, r.applyMatrix((new t.Matrix4).makeScale(1, 1, -1)), r
    }, n.prototype._transition = function(e, i) {
        var o;
        if (this.prop.isCubemap ? (i && (console.warn("PhotoSphereViewer: cannot perform cubemap transition to different position."), i = void 0), (o = this._createCubemap(.9)).material.forEach(function(t, i) {
                t.map = e[i], t.transparent = !0, t.opacity = 0
            })) : ((o = this._createSphere(.9)).material.map = e, o.material.transparent = !0, o.material.opacity = 0), i) {
            ...
            o.quaternion.multiplyQuaternions(s, o.quaternion), (this.config.latitude_range || this.config.longitude_range) && (this.config.longitude_range = this.config.latitude_range = null, console.warn("PhotoSphereViewer: trying to perform transition with longitude_range and/or latitude_range, ranges cleared."))
        }
        return this.scene.add(o), this.needsUpdate(), x.animation({
            properties: {
                opacity: {
                    start: 0,
                    end: 1
            ...
            this._setTexture(e), this.scene.remove(o), o.geometry.dispose(), o.geometry = null, i && this.rotate(i)
        }.bind(this))
        ...
        this.config.longitude_range = null, x.animation({
            ...
            t.config.longitude_range = i, t.config.anim_speed = e
        })
    }, n.prototype._putPanoramaCache = function(t) {
        if (!this.config.cache_texture) throw new E("Cannot add panorama to cache, cache_texture is disabled");
    ...
    }, n.MOVE_THRESHOLD = 4, n.ANGLE_THRESHOLD = .003, n.DBLCLICK_DELAY = 300, n.INERTIA_WINDOW = 300, n.SPHERE_RADIUS = 100, n.SPHERE_VERTICES = 64, n.CUBE_VERTICES = 8, n.CUBE_MAP = [0, 2, 4, 5, 3, 1], n.CUBE_HASHMAP = ["left", "right", "top", "bottom", "back", "front"], n.KEYMAP = {
        33: "PageUp",
        34: "PageDown",
        37: "ArrowLeft",
        38: "ArrowUp",
        ...
        1 === t.touches.length ? (t.preventDefault(), this._move(t.touches[0])) : 2 === t.touches.length && (t.preventDefault(), this._zoom(t))
    }, n.prototype._startMove = function(t) {
        this.stopAutorotate(), this.stopAnimation(), this.prop.mouse_x = this.prop.start_mouse_x = parseInt(t.clientX), this.prop.mouse_y = this.prop.start_mouse_y = parseInt(t.clientY), this.prop.moving = !0, this.prop.zooming = !1, this.prop.mouse_history.length = 0, this._logMouseMove(t)
        ...
        if (o) {
            ...
            if (i.longitude = s.longitude, i.latitude = s.latitude, !this.prop.isCubemap) {
                ...
                i.texture_x = r.x, i.texture_y = r.y
            }
            this.prop.dblclick_timeout ? (Math.abs(this.prop.dblclick_data.client_x - i.client_x) < n.MOVE_THRESHOLD && Math.abs(this.prop.dblclick_data.client_y - i.client_y) < n.MOVE_THRESHOLD && this.trigger("dblclick", this.prop.dblclick_data), clearTimeout(this.prop.dblclick_timeout), this.prop.dblclick_timeout = null, this.prop.dblclick_data = null) : (this.trigger("click", i), this.prop.dblclick_data = x.clone(i), this.prop.dblclick_timeout = setTimeout(function() {
        ...
        if (this.prop.zooming) {
            ...
            this.zoom(this.prop.zoom_lvl + o), this.prop.pinch_dist = i
        }
        ...
        for (var i = null, o = 0; o < this.prop.mouse_history.length;) this.prop.mouse_history[0][o] < e - n.INERTIA_WINDOW ? this.prop.mouse_history.splice(o, 1) : i && this.prop.mouse_history[0][o] - i > n.INERTIA_WINDOW / 10 ? (this.prop.mouse_history.splice(0, o), o = 0, i = this.prop.mouse_history[0][o]) : (o++, i = this.prop.mouse_history[0][o])
    }, n.prototype.load = function() {
        if (!this.config.panorama) throw new E("No value given for panorama.");
        ...
        if (null !== this.prop.loading_promise) throw new E("Loading already in progress");
        return "boolean" == typeof e && (i = e, e = void 0), e && (this.cleanPosition(e), this._stopAll()), this.config.panorama = t, i && this.config.transition && this.scene ? (this.config.transition.loader && this.loader.show(), this.prop.loading_promise = this._loadTexture(this.config.panorama).then(function(t) {
            return this.loader.hide(), this._transition(t, e)
        }.bind(this)).ensure(function() {
            this.loader.hide(), this.prop.loading_promise = null
        }.bind(this)).rethrow()) : (this.loader.show(), this.canvas_container && (this.canvas_container.style.opacity = 0), this.prop.loading_promise = this._loadTexture(this.config.panorama).then(function(t) {
            this._setTexture(t), e && this.rotate(e)
        }.bind(this)).ensure(function() {
            this.loader.hide(), this.canvas_container.style.opacity = 1, this.prop.loading_promise = null
        }.bind(this)).rethrow()), this.prop.loading_promise
        ...
        var t, e;
        return function(i) {
            e = void 0 === t ? 0 : i - t, t = i, this.rotate({
        ...
        if (x.checkTHREE("DeviceOrientationControls")) return n.SYSTEM.deviceOrientationSupported.then(function() {
            this._stopAll(), this.doControls = new t.DeviceOrientationControls(this.camera), this.doControls.alphaOffset = this.prop.position.longitude, this.doControls.update();
            ...
            return console.warn("PhotoSphereViewer: gyroscope not available"), e.rejected()
        });
        ...
        return function() {
            this.doControls.alphaOffset = this.prop.gyro_alpha_offset, this.doControls.update(), this.camera.getWorldDirection(this.prop.direction), this.prop.direction.multiplyScalar(n.SPHERE_RADIUS);
        ...
        if (!x.checkTHREE("DeviceOrientationControls", "StereoEffect")) throw new E("Missing Three.js components: StereoEffect, DeviceOrientationControls. Get them from three.js-examples package.");
        this.startNoSleep(), this.enterFullscreen(), this.lockOrientation(), this.startGyroscopeControl().then(function() {
            this.stereoEffect = new t.StereoEffect(this.renderer), this.needsUpdate(), this.hud.hide(), this.navbar.hide(), this.panel.hidePanel(), this.trigger("stereo-updated", !0), this.notification.showNotification({
        ...
        var t = function() {
            window.innerHeight > window.innerWidth && (this.pleaseRotate || (this.pleaseRotate = new u(this)), this.pleaseRotate.show())
        };
        ...
        this.cleanPosition(t), this.applyRanges(t).forEach(this.trigger.bind(this, "_side-reached")), this.prop.position.longitude = t.longitude, this.prop.position.latitude = t.latitude, this.needsUpdate(), this.trigger("position-updated", this.getPosition())
    }, n.prototype.animate = function(t, i) {
        if (this._stopAll(), this.cleanPosition(t), !i || Math.abs(t.longitude - this.prop.position.longitude) < n.ANGLE_THRESHOLD && Math.abs(t.latitude - this.prop.position.latitude) < n.ANGLE_THRESHOLD) return this.rotate(t), e.resolved();
        if (this.applyRanges(t).forEach(this.trigger.bind(this, "_side-reached")), !i && "number" != typeof i) {
        ...
        this.prop.zoom_lvl = x.bound(Math.round(e), 0, 100), this.prop.vFov = this.config.max_fov + this.prop.zoom_lvl / 100 * (this.config.min_fov - this.config.max_fov), this.prop.hFov = t.Math.radToDeg(2 * Math.atan(Math.tan(t.Math.degToRad(this.prop.vFov) / 2) * this.prop.aspect)), this.needsUpdate(), this.trigger("zoom-updated", this.getZoomLevel())
        ...
        if (!this.config.cache_texture) throw new E("Cannot preload panorama, cache_texture is disabled");
        return this._loadTexture(t)
    }, n.prototype.clearPanoramaCache = function(t) {
        if (!this.config.cache_texture) throw new E("Cannot clear cache, cache_texture is disabled");
        if (t) {
        ...
        if (!this.config.cache_texture) throw new E("Cannot query cache, cache_texture is disabled");
        ...
        ["width", "height"].forEach(function(e) {
            t[e] && (/^[0-9.]+$/.test(t[e]) && (t[e] += "px"), this.parent.style[e] = t[e])
        }, this)
    }, n.prototype.textureCoordsToSphericalCoords = function(t) {
        if (this.prop.isCubemap) throw new E("Unable to use texture coords with cubemap.");
        ...
        t.hasOwnProperty("x") && t.hasOwnProperty("y") && x.deepmerge(t, this.textureCoordsToSphericalCoords(t)), t.longitude = x.parseAngle(t.longitude), t.latitude = x.parseAngle(t.latitude, !0)
    }, n.prototype.applyRanges = function(e) {
        var i, o, n = [];
        return this.config.longitude_range && (i = x.clone(this.config.longitude_range), o = t.Math.degToRad(this.prop.hFov) / 2, i[0] = x.parseAngle(i[0] + o), i[1] = x.parseAngle(i[1] - o), i[0] > i[1] ? e.longitude > i[1] && e.longitude < i[0] && (e.longitude > i[0] / 2 + i[1] / 2 ? (e.longitude = i[0], n.push("left")) : (e.longitude = i[1], n.push("right"))) : e.longitude < i[0] ? (e.longitude = i[0], n.push("left")) : e.longitude > i[1] && (e.longitude = i[1], n.push("right"))), this.config.latitude_range && (i = x.clone(this.config.latitude_range), o = t.Math.degToRad(this.prop.vFov) / 2, i[0] = x.parseAngle(Math.min(i[0] + o, i[1]), !0), i[1] = x.parseAngle(Math.max(i[1] - o, i[0]), !0), e.latitude < i[0] ? (e.latitude = i[0], n.push("bottom")) : e.latitude > i[1] && (e.latitude = i[1], n.push("top"))), n
    };

    function s(t) {
        this.psv = t instanceof n ? t : t.psv, this.parent = t, this.container = null, this.visible = !0, this.constructor.publicMethods && this.constructor.publicMethods.forEach(function(t) {
    ...
    r.prototype = Object.create(s.prototype), r.prototype.constructor = r, r.className = "psv-hud", r.publicMethods = ["addMarker", "removeMarker", "updateMarker", "clearMarkers", "getMarker", "getCurrentMarker", "gotoMarker", "hideMarker", "showMarker", "toggleMarker", "toggleMarkersList", "showMarkersList", "hideMarkersList"], r.prototype.create = function() {
        s.prototype.create.call(this), this.svgContainer = document.createElementNS(x.svgNS, "svg"), this.svgContainer.setAttribute("class", "psv-hud-svg-container"), this.container.appendChild(this.svgContainer), this.container.addEventListener("mouseenter", this, !0), this.container.addEventListener("mouseleave", this, !0), this.container.addEventListener("mousemove", this, !0), this.psv.on("click", this), this.psv.on("dblclick", this), this.psv.on("render", this), this.psv.on("open-panel", this), this.psv.on("close-panel", this)
        ...
        if (!t.id) throw new E("missing marker id");
        if (this.markers[t.id]) throw new E('marker "' + t.id + '" already exists');
        ...
        if (!this.markers[e]) throw new E('cannot find marker "' + e + '"');
        ...
        this.prop.panelOpening = !0, this.psv.panel.showPanel(e, !0), this.psv.panel.container.querySelector(".psv-markers-list").addEventListener("click", this._onClickItem.bind(this))
        ...
        if (t._dynamicSize) {
            ...
            t.$el.style.transform = e, x.toggleClass(t.$el, "psv-marker--transparent", !1), t.width = i.right - i.left, t.height = i.bottom - i.top
        }
        ...
        return e.forEach(function(t) {
            i = Math.min(i, t.x), o = Math.min(o, t.y), n = Math.max(n, t.x), s = Math.max(s, t.y)
        }), t.width = n - i, t.height = s - o, {
            x: i,
            y: o
        }
        ...
        if (t.target && (e = t.target.psvMarker)) {
            if (e.isPoly() && t.relatedTarget && x.hasParent(t.relatedTarget, this.psv.tooltip.container)) return;
    ...
    a.prototype = Object.create(s.prototype), a.prototype.constructor = a, a.className = "psv-loader-container", a.prototype.create = function() {
        s.prototype.create.call(this);
        var t = n.SYSTEM.pixelRatio;
        this.loader = document.createElement("div"), this.loader.className = "psv-loader", this.container.appendChild(this.loader), this.canvas = document.createElement("canvas"), this.canvas.className = "psv-loader-canvas", this.canvas.width = this.loader.clientWidth * t, this.canvas.height = this.loader.clientWidth * t, this.loader.appendChild(this.canvas), this.tickness = (this.loader.offsetWidth - this.loader.clientWidth) / 2 * t;
        var e;
        if (this.psv.config.loading_img ? ((e = document.createElement("img")).className = "psv-loader-image", e.src = this.psv.config.loading_img) : this.psv.config.loading_txt && ((e = document.createElement("div")).className = "psv-loader-text", e.innerHTML = this.psv.config.loading_txt), e) {
            var i = Math.round(Math.sqrt(2 * Math.pow((this.canvas.width / 2 - this.tickness / 2) / t, 2)));
            e.style.maxWidth = i + "px", e.style.maxHeight = i + "px", this.loader.appendChild(e)
        }
    }, a.prototype.destroy = function() {
        delete this.loader, delete this.canvas, s.prototype.destroy.call(this)
    }, a.prototype.setProgress = function(t) {
        var e = this.canvas.getContext("2d");
        e.clearRect(0, 0, this.canvas.width, this.canvas.height), e.lineWidth = this.tickness, e.strokeStyle = x.getStyle(this.loader, "color"), e.beginPath(), e.arc(this.canvas.width / 2, this.canvas.height / 2, this.canvas.width / 2 - this.tickness / 2, -Math.PI / 2, t / 100 * 2 * Math.PI - Math.PI / 2), e.stroke()
    };

    function h(t) {
        if (s.call(this, t), this.config = this.psv.config.navbar, this.items = [], !0 === this.config) this.config = x.clone(n.DEFAULTS.navbar);
            ...
            var e = this.config;
            this.config = [], x.forEach(e, function(t, e) {
    ...
    h.prototype = Object.create(s.prototype), h.prototype.constructor = h, h.className = "psv-navbar psv-navbar--open", h.publicMethods = ["showNavbar", "hideNavbar", "toggleNavbar", "getNavbarButton"], h.prototype.create = function() {
        s.prototype.create.call(this), this.config.forEach(function(t) {
            if ("object" == typeof t) this.items.push(new m(this, t));
                ...
                case "caption":
                    ...
                    break;
                default:
                    if (0 !== t.indexOf("spacer")) throw new E("Unknown button " + t);
        ...
        return this.items.some(function(e) {
            return e.id === t && (i = e, !0)
        }), i || e || console.warn('PhotoSphereViewer: button "' + t + '" not found in the navbar.'), i
    ...
    function p(t, e) {
        s.call(this, t), this.content = null, this.icon = null, this.prop = {
            caption: "",
            width: 0,
            hidden: !1
        }, this.create(), this.setCaption(e)
    }
    p.prototype = Object.create(s.prototype), p.prototype.constructor = p, p.className = "psv-caption", p.publicMethods = ["setCaption"], p.prototype.create = function() {
        s.prototype.create.call(this), this.container.innerHTML = n.ICONS["info.svg"], this.icon = this.container.querySelector("svg"), this.icon.setAttribute("class", "psv-caption-icon"), this.icon.style.display = "none", this.content = document.createElement("span"), this.content.className = "psv-caption-content", this.container.appendChild(this.content), this.icon.addEventListener("click", this), window.addEventListener("resize", this)
        ...
        parseInt(x.getStyle(this.container, "width")) >= this.prop.width ? (this.icon.style.display = "none", this.content.style.display = "") : (this.icon.style.display = "", this.content.style.display = "none")
    ...
    c.prototype = Object.create(s.prototype), c.prototype.constructor = c, c.className = "psv-notification", c.publicMethods = ["showNotification", "hideNotification", "isNotificationVisible"], c.prototype.create = function() {
    ...
    l.prototype = Object.create(s.prototype), l.prototype.constructor = l, l.className = "psv-panel", l.publicMethods = ["showPanel", "hidePanel"], l.prototype.create = function() {
        s.prototype.create.call(this), this.container.innerHTML = '<div class="psv-panel-resizer"></div><div class="psv-panel-close-button"></div><div class="psv-panel-content"></div>', this.content = this.container.querySelector(".psv-panel-content");
        this.container.querySelector(".psv-panel-close-button").addEventListener("click", this.hidePanel.bind(this)), this.psv.config.mousewheel && this.container.addEventListener(n.SYSTEM.mouseWheelEvent, function(t) {
            t.stopPropagation()
        });
        ...
        this.content.innerHTML = t, this.content.scrollTop = 0, this.container.classList.add("psv-panel--open"), x.toggleClass(this.content, "psv-panel-content--no-margin", !0 === e), this.prop.opened = !0, this.psv.trigger("open-panel")
    ...
    u.prototype = Object.create(s.prototype), u.prototype.constructor = u, u.className = "psv-please-rotate", u.prototype.create = function() {
        s.prototype.create.call(this), this.container.innerHTML = '<div class="psv-please-rotate-image">' + n.ICONS["mobile-rotate.svg"] + '</div><div class="psv-please-rotate-text">' + this.psv.config.lang.please_rotate[0] + '</div><div class="psv-please-rotate-subtext">' + this.psv.config.lang.please_rotate[1] + "</div>", this.container.addEventListener("click", this), window.addEventListener("orientationchange", this)
    }, u.prototype.destroy = function() {
        window.removeEventListener("orientationchange", this), s.prototype.destroy.call(this)
    ...
    d.prototype = Object.create(s.prototype), d.prototype.constructor = d, d.className = "psv-tooltip", d.publicMethods = ["showTooltip", "hideTooltip", "isTooltipVisible"], d.leftMap = {
        0: "left",
        .5: "center",
        1: "right"
    }, d.topMap = {
        ...
        if ("center" === t.position[0] && "center" === t.position[1]) throw new E('unable to parse tooltip position "center center"');
        if (e)
        ...
        t.className && x.addClasses(i, t.className), o.innerHTML = t.content, i.style.top = "0px", i.style.left = "0px";
        ...
        switch (t.posClass[1]) {
            case "right":
                i ? (t.left = e.left + e.box.width / 2 - this.config.offset - this.config.arrow_size, t.arrow_left = this.config.offset) : (t.left = e.left + e.box.width + this.config.offset + this.config.arrow_size, t.arrow_left = 2 * -this.config.arrow_size);
                break;
            case "center":
                t.left = e.left + e.box.width / 2 - t.width / 2, t.arrow_left = t.width / 2 - this.config.arrow_size;
                break;
            case "left":
                i ? (t.left = e.left - t.width + e.box.width / 2 + this.config.offset + this.config.arrow_size, t.arrow_left = t.width - this.config.offset - 2 * this.config.arrow_size) : (t.left = e.left - t.width - this.config.offset - this.config.arrow_size, t.arrow_left = t.width)
        }
    };
    ...
    f.prototype = Object.create(s.prototype), f.prototype.constructor = f, f.id = null, f.icon = null, f.iconActive = null, f.prototype.create = function() {
        ...
        x.toggleClass(this.container, "psv-button--active", t), this.constructor.iconActive && this._setIcon(t ? this.constructor.iconActive : this.constructor.icon)
        ...
        e || (e = this.container), t ? (e.innerHTML = n.ICONS[t], e.querySelector("svg").setAttribute("class", "psv-button-svg")) : e.innerHTML = ""
    }, f.prototype._onClick = function() {};

    function g(t) {
        f.call(this, t), this.create()
    }
    g.prototype = Object.create(f.prototype), g.prototype.constructor = g, g.id = "autorotate", g.className = "psv-button psv-button--hover-scale psv-autorotate-button", g.icon = "play.svg", g.iconActive = "play-active.svg", g.prototype.create = function() {
    ...
    m.prototype = Object.create(f.prototype), m.prototype.constructor = m, m.className = "psv-button psv-custom-button", m.prototype.create = function() {
        f.prototype.create.call(this), this.config.className && x.addClasses(this.container, this.config.className), this.config.title && (this.container.title = this.config.title), this.config.content && (this.container.innerHTML = this.config.content), !1 !== this.config.enabled && !0 !== this.config.disabled || this.disable(), !1 !== this.config.visible && !0 !== this.config.hidden || this.hide()
    ...
    v.prototype = Object.create(f.prototype), v.prototype.constructor = v, v.id = "download", v.className = "psv-button psv-button--hover-scale psv-download-button", v.icon = "download.svg", v.prototype._onClick = function() {
    ...
    y.prototype = Object.create(f.prototype), y.prototype.constructor = y, y.id = "fullscreen", y.className = "psv-button psv-button--hover-scale psv-fullscreen-button", y.icon = "fullscreen-in.svg", y.iconActive = "fullscreen-out.svg", y.prototype.create = function() {
        f.prototype.create.call(this), n.SYSTEM.fullscreenEvent || (this.hide(), console.warn("PhotoSphereViewer: fullscreen not supported.")), this.psv.on("fullscreen-updated", this)
    ...
    _.prototype = Object.create(f.prototype), _.prototype.constructor = _, _.id = "gyroscope", _.className = "psv-button psv-button--hover-scale psv-gyroscope-button", _.icon = "compass.svg", _.prototype.create = function() {
        f.prototype.create.call(this), n.SYSTEM.deviceOrientationSupported.then(this._onAvailabilityChange.bind(this, !0), this._onAvailabilityChange.bind(this, !1)), this.hide(), this.psv.on("gyroscope-updated", this)
    ...
    w.prototype = Object.create(f.prototype), w.prototype.constructor = w, w.id = "markers", w.className = "psv-button psv-button--hover-scale psv-markers-button", w.icon = "pin.svg", w.prototype._onClick = function() {
    ...
    b.prototype = Object.create(f.prototype), b.prototype.constructor = b, b.id = "stereo", b.className = "psv-button psv-button--hover-scale psv-stereo-button", b.icon = "stereo.svg", b.prototype.create = function() {
        f.prototype.create.call(this), n.SYSTEM.deviceOrientationSupported.then(this._onAvailabilityChange.bind(this, !0), this._onAvailabilityChange.bind(this, !1)), this.hide(), this.psv.on("stereo-updated", this)
    ...
    M.prototype = Object.create(f.prototype), M.prototype.constructor = M, M.id = "zoom", M.className = "psv-button psv-zoom-button", M.prototype.create = function() {
    ...
    function E(t) {
        this.message = t, "captureStackTrace" in Error ? Error.captureStackTrace(this, E) : this.stack = (new Error).stack
    }
    E.prototype = Object.create(Error.prototype), E.prototype.name = "PSVError", E.prototype.constructor = E, n.Error = E;

    function k(t, e) {
        if (!t.id) throw new E("missing marker id");
        if (t.image && (!t.width || !t.height)) throw new E("missing marker width/height");
        if ((t.image || t.html) && !(t.hasOwnProperty("x") && t.hasOwnProperty("y") || t.hasOwnProperty("latitude") && t.hasOwnProperty("longitude"))) throw new E("missing marker position, latitude/longitude or x/y");
        ...
        }), (i = this.isNormal() ? document.createElement("div") : this.isPolygon() ? document.createElementNS(x.svgNS, "polygon") : this.isPolyline() ? document.createElementNS(x.svgNS, "polyline") : document.createElementNS(x.svgNS, this.type)).id = "psv-marker-" + this.id, i.psvMarker = this, this.update(t)
    }
    k.types = ["image", "html", "polygon_px", "polygon_rad", "polyline_px", "polyline_rad", "rect", "circle", "ellipse", "path"], k.getType = function(t, e) {
        var i = [];
        if (k.types.forEach(function(e) {
                t[e] && i.push(e)
            }), 0 === i.length && !e) throw new E("missing marker content, either " + k.types.join(", "));
        if (i.length > 1) throw new E("multiple marker content, either " + k.types.join(", "));
        return i[0]
        ...
        if (t && t !== this) {
            var e = k.getType(t, !0);
            if (void 0 !== e && e !== this.type) throw new E("cannot change marker type");
            x.deepmerge(this, t)
        }
        ...
        })), this.style && x.deepmerge(this.$el.style, this.style), this.anchor = x.parsePosition(this.anchor), this.isNormal() ? this._updateNormal() : this.isPolygon() ? this._updatePoly("polygon_rad", "polygon_px") : this.isPolyline() ? this._updatePoly("polyline_rad", "polyline_px") : this._updateSvg()
    ...
    var x = {};
    return n.Utils = x, x.TwoPI = 2 * Math.PI, x.HalfPI = Math.PI / 2, x.svgNS = "http://www.w3.org/2000/svg", x.checkTHREE = function(e) {
            for (var i = 0, o = arguments.length; i < o; i++)
                if (!(arguments[i] in t)) return !1;
            return !0
            ...
            if ("DeviceOrientationEvent" in window) {
                var i = function(e) {
                    e && null !== e.alpha && !isNaN(e.alpha) ? t.resolve() : t.reject(), window.removeEventListener("deviceorientation", i)
                };
                window.addEventListener("deviceorientation", i, !1), setTimeout(function() {
            ...
            var t = e(),
                i = function(e) {
                    e ? t.resolve() : t.reject(), window.removeEventListener("touchstart", i)
                };
            return window.addEventListener("touchstart", i, !1), setTimeout(function() {
                t.promise.isPending() && i(null)
            }, 1e4), t.promise
            ...
            if (t.classList) void 0 === i ? t.classList.toggle(e) : i && !t.classList.contains(e) ? t.classList.add(e) : i || t.classList.remove(e);
                ...
                var o = t.getAttribute("class") || "",
                    n = -1 !== o.indexOf(e),
                    s = new RegExp("(?:^|\\s)" + e + "(?:\\s|$)");
                void 0 !== i && !i || n ? i || (o = o.replace(s, " ")) : o += o.length > 0 ? " " + e : e, t.setAttribute("class", o)
            }
            ...
            return [0, x.TwoPI, -x.TwoPI].reduce(function(i, o) {
                return o = e - t + o, Math.abs(o) < Math.abs(i) ? o : i
            }, 1 / 0)
            ...
            if ("string" == typeof e) {
                ...
                switch (o.match(/(pm|per minute)$/) && (i /= 60), o) {
                    case "dpm":
            ...
            if ("string" == typeof e) {
                ...
                if (!o) throw new E('unknown angle "' + e + '"');
            ...
            return (e = (i ? e + Math.PI : e) % x.TwoPI) < 0 && (e = x.TwoPI + e), i ? x.bound(e - Math.PI, -x.HalfPI, x.HalfPI) : e
        }, x.cleanTHREEScene = function(e) {
            e.children.forEach(function(e) {
                e instanceof t.Mesh && (e.geometry && (e.geometry.dispose(), e.geometry = null), e.material && (e.material.materials ? (e.material.materials.forEach(function(t) {
                    t.map && (t.map.dispose(), t.map = null), t.dispose()
                }), e.material.materials.length = 0) : (e.material.map && (e.material.map.dispose(), e.material.map = null), e.material.dispose()), e.material = null))
            }), e.children.length = 0
            ...
            function n(e) {
                if (-1 !== i.promise.getStatus()) {
                    null === o && (o = e);
                    var s, r = (e - o) / t.duration,
                        a = {};
                    if (r < 1) {
                        for (s in t.properties) a[s] = t.properties[s].start + (t.properties[s].end - t.properties[s].start) * t.easing(r);
                        t.onTick(a, r), window.requestAnimationFrame(n)
                    } else {
                        for (s in t.properties) a[s] = t.properties[s].end;
                        t.onTick(a, 1), window.requestAnimationFrame(function() {
                            i.resolve()
                        })
                    }
                }
            ...
            var i, o, n, s = null,
                r = 0,
                a = function() {
                    r = Date.now(), s = null, n = t.apply(i, o), s || (i = o = null)
                };
            return function() {
                var h = Date.now();
                r || (r = h);
                var p = e - (h - r);
                return i = this, o = arguments, p <= 0 || p > e ? (s && (clearTimeout(s), s = null), r = h, n = t.apply(i, o), s || (i = o = null)) : s || (s = setTimeout(a, p)), n
            }
            ...
            return function t(e, o) {
                return Array.isArray(o) ? (e && Array.isArray(e) ? e.length = 0 : e = [], o.forEach(function(i, o) {
                    e[o] = t(null, i)
                })) : "object" == typeof o ? (e && !Array.isArray(e) || (e = {}), Object.keys(o).forEach(function(n) {
